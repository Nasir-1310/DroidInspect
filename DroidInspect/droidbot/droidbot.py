...
import subprocess #for screenshot check
import json       #for screenshot check
...
class DroidBot(object):
    ...
    def start(self):
        ...
        #      def call_screenshot_checker(self, state, event):
        #           self.screenshot_count += 1
        #           device_data = json.dumps(self.device.__dict__)  # Serialize the device object
        #           result = subprocess.run(
        #             [
        #              "python", "droidbot_screenshot_checker.py", device_data, self.output_dir, state, event, str(self.screenshot_count)
        #             ],
        #         capture_output=True,
        #      text=True
        #   )
